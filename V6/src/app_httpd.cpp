// stipped live cam server for the Medusa project
// Marco Schikatis 2020
// Original @espressif
...
httpd_handle_t back_httpd = NULL;
...
static esp_err_t capture_handler(httpd_req_t *req){
  ...
  //  int face_id = 0;
    ...
    if (!image_matrix) {
        esp_camera_fb_return(fb);
        Serial.println("dl_matrix3du failed");
        httpd_resp_send_500(req);
        return ESP_FAIL;
    }
...
void startCameraServer(){
   ...
   // Serial.printf("Starting stream server on port: '%d'\n", config.server_port);
    if (httpd_start(&stream_httpd, &config) == ESP_OK) {
        ...
        httpd_register_uri_handler(back_httpd, &back_uri);
    }
}
